...
def get_model_base(architecture, backbone):
    ...
    if 'daformer_' in architecture and ('mitb5' in backbone or 'convnext' in backbone):
...
def get_backbone_cfg(backbone):
    ...
    #----------freeze base dino first 2 stages---------------
    if backbone == 'convnext_base_dinov3_lvd1689m':
        return dict(
            type='TimmBackbone',
            model_name='hf-hub:timm/convnext_base.dinov3_lvd1689m',
            pretrained=True,
            frozen=True,
            unfreeze_stage_ids=(2, 3),
            out_indices=(0, 1, 2, 3),
            )
...
def generate_experiment_cfgs(id):
    ...
    def config_from_vars():
        ...
        # }
        pretr = get_pretraining_file(backbone)
        cfg['model'] = {
            'backbone': get_backbone_cfg(backbone),
        }
        if pretr is not None:
            cfg['model']['pretrained'] = pretr  # keep old behavior for mit/resnet
        # ✅ ConvNeXt-Base feature dims for DAFormerHead
        if backbone  in ['convnext_base_in22k', 'convnext_base_dinov3_lvd1689m']:
            cfg['model'].setdefault('decode_head', {})
            cfg['model']['decode_head']['in_channels'] = [128, 256, 512, 1024]
            cfg['model']['decode_head']['in_index'] = [0, 1, 2, 3]
        if backbone == 'convnext_large_dinov3_lvd1689m':
            cfg['model'].setdefault('decode_head', {})
            cfg['model']['decode_head']['in_channels'] = [192, 384, 768, 1536]
            cfg['model']['decode_head']['in_index'] = [0, 1, 2, 3]
        ...
        if mask_mode is not None:
            ...
            # cfg['uda']['intra_mix_lambda'] = intra_mix_lambda
            # oritinal block mask
        ...
        if 'convnext' in backbone:
            opt_param_cfg['backbone'] = dict(lr_mult=0.1)
            opt_param_cfg['model.backbone'] = dict(lr_mult=0.1)   # sometimes needed
            opt_param_cfg['norm'] = dict(decay_mult=0.)
    ...
    # -------------------------------------------------------------------------
    # ICM with HRDA and MIC for CS->ACDC UDA Benchmarks (Table 2)
    # -------------------------------------------------------------------------

    if id == 100: # dino v3 backbone
        iters = 40000
        seeds = [0, 1, 2]
        architecture, backbone = 'hrda1-512-0.1_daformer_sepaspp', 'convnext_base_dinov3_lvd1689m'
