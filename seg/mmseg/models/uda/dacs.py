...
from mmseg.models.uda.mix_consistency_module import \
    IntraClassMixConsistencyModule
...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        self.mic = None
        self.intra_mix = IntraClassMixConsistencyModule(require_teacher=False, cfg=cfg)
    ...
    def update_debug_state(self):
        ...
        if self.intra_mix is not None:
            self.intra_mix.debug = debug
    ...
    def forward_train(self,
        ...
        if self.intra_mix is not None:
            self.intra_mix.update_weights(self.get_model(), self.local_iter)
        ...
        if self.intra_mix is not None:
            intra_mix_loss = self.intra_mix(self.get_model(), img, img_metas,
                                gt_semantic_seg, target_img,
                                target_img_metas, valid_pseudo_mask,
                                pseudo_label, pseudo_weight)

            seg_debug.update(self.intra_mix.debug_output)
            intra_mix_loss = add_prefix(intra_mix_loss, 'intra_mix')
            intra_mix_loss, intra_mix_log_vars = self._parse_losses(intra_mix_loss)
            log_vars.update(intra_mix_log_vars)
            intra_mix_loss.backward()
