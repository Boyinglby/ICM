...
class EMATeacher(Module):
    ...
    # added 260219, to enable KL(teacher(clean),student(mix)) --------------------     
    def forward_logits(self, img, img_metas):
        """Return EMA teacher logits (B,K,H,W) with dropout/droppath disabled."""
        self.update_debug_state()

        # Disable stochastic layers (same as in __call__)
        ...
        with torch.no_grad():
            # generate_pseudo_label returns logits in your codebase
            ema_logits = self.get_ema_model().generate_pseudo_label(img, img_metas)

        # keep debug output consistent
